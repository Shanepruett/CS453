...
static ssize_t booga_read(struct file *filp, char *buf, size_t count,
			  loff_t * f_pos);
static ssize_t booga_write(struct file *filp, const char *buf,
...
static ssize_t
booga_read(struct file *filp, char *buf, size_t count, loff_t * f_pos)
{
...
static ssize_t
booga_write(struct file *filp, const char *buf, size_t count,
	    loff_t * f_pos)
{
...
static int __init booga_init(void)
{
    ...
    booga_device =
	(struct booga_dev *) kmalloc(sizeof(struct booga_dev), GFP_KERNEL);
    if (!booga_device) {
	result = -ENOMEM;
	goto fail_malloc;
    }
